...
export default function DashboardPage() {
  ...
  // Calculate stats
  const relevantLeads = isAdmin ? leads : leads.filter(l => l.assignedTo === user?.id);
                ...
                .map((lead) => {
                  ...
                  return (
                    <tr key={lead.id} className="hover:bg-[#F8F9FB] transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <Link href={`/leads/${lead.id}`} className="flex items-center gap-3 hover:text-[#F34147]">
                          <div className="h-8 w-8 rounded-full bg-[#F34147]/10 text-[#F34147] flex items-center justify-center font-medium text-xs">
                            {lead.name.charAt(0)}
                          </div>
                          <span className="font-semibold text-[#0D0D12]">{lead.name}</span>
                        </Link>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <Badge
                          variant="secondary"
                          className="rounded-md px-2.5 py-1 font-medium border-0 capitalize"
                        >
                          {lead.stage}
                        </Badge>
                      </td>
