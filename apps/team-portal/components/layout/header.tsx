    ...
    <header className="flex h-[80px] items-center justify-between px-8 bg-white border-b border-[#DFE1E7]">
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="flex items-center justify-center h-5 w-5 bg-[#ECEFF3] rounded-[4px]">
            <span className="text-[13px] font-semibold text-[#818898] leading-none">
              K
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex items-center gap-2 mr-2 bg-gray-100 p-1 rounded-lg">
          <Button
            variant={user?.role === "superadmin" ? "default" : "ghost"}
            size="sm"
            className={`h-7 text-xs ${
              user?.role === "superadmin"
                ? "bg-[#F34147] text-white hover:bg-[#D93036]"
                : "text-[#666D80]"
            }`}
          ...
          <Button
            variant={user?.role === "sales_executive" ? "default" : "ghost"}
            size="sm"
            className={`h-7 text-xs ${
        ...
        <Popover>
          ...
          <PopoverContent
            className="w-[400px] p-0 rounded-[16px] shadow-[0px_16px_32px_-1px_rgba(128,136,151,0.2)] border-[#DFE1E7]"
            align="end"
          >
                ...
                <div key={notification.id} className="flex gap-3 items-start">
                  ...
                  <div className="flex flex-col gap-1 flex-1">
                    <div className="flex items-center justify-between">
                      <h4 className="text-[14px] font-semibold text-[#0D0D12]">
                        {notification.title}
                      </h4>
                      {!notification.read && (
                    ...
                    <p className="text-[12px] text-[#666D80] leading-[1.5]">
            ...
            <div className="p-6 border-t border-[#DFE1E7] flex items-center justify-between">
              <button className="text-[16px] font-semibold text-[#F34147]">
                Mark as all read
              </button>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 outline-none">
              <Avatar className="h-8 w-8 border border-white shadow-sm relative">
              ...
              <div className="flex flex-col items-start leading-tight">
                ...
                <span className="text-xs font-normal text-[#666D80]">
                  {user?.role === "superadmin" ? "Super Admin" : "Sales Executive"}
                </span>
              </div>
      ...
      <SignOutDialog open={logoutOpen} onOpenChange={setLogoutOpen} onConfirm={logout} />
    </header>
  );
}
