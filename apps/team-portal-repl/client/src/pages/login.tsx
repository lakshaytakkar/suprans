...
export default function Login() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // For now, we'll just log them in as a sales executive by default
      // or admin if they use specific credentials (optional)
      
      const role = email.includes("admin") ? "superadmin" : "sales_executive";
      
      setCurrentUser({
        id: role === "superadmin" ? "u3" : "u1",
        name: role === "superadmin" ? "Admin User" : "Rahul Sharma",
        email: email || "rahul@suprans.in",
        role: role,
        ...
        <div className="bg-white w-full max-w-[480px] rounded-2xl p-10 shadow-[0px_4px_24px_rgba(0,0,0,0.04)] border border-[#DFE1E7] relative z-20">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <a href="#" className="text-sm font-medium text-[#F34147] hover:text-[#D93036] transition-colors">
                Forgot Password?
              </a>
            </div>
          ...
          <div className="mt-8 text-center text-[15px] text-[#666D80]">
            Don't have an account?{" "}
            <a href="#" className="font-semibold text-[#F34147] hover:text-[#D93036] transition-colors">Register</a>
          </div>
        </div>
      </div>
